...
def run_fedvim(args: argparse.Namespace) -> Path:
    """Run the fixed-k FedViM evaluation protocol from federated ID statistics."""
    ...
    cov_global = federated_stats["cov"]
    ...
    if args.alpha_method != "empirical":
        raise ValueError("FedViM evaluation now requires empirical alpha recomputation from client ID-train splits.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Post-hoc FedViM requires empirical alpha from client ID-train splits.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
