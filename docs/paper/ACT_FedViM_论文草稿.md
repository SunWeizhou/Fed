...
第三，平均值会受到个别模型的显著影响。本文中 `DenseNet169` 上的较大差异明显拉高了 ACT 的平均 AUROC；若剔除这一模型，ACT 相对 FedViM 的平均变化约为 Near `-0.40` 个百分点、Far `-0.79` 个百分点。此外，正文结果目前基于单次种子实验，小幅涨跌仍可能受到训练噪声、联邦划分和经验 `alpha` 校准的影响。
