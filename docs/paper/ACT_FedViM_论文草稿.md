...
图 5-2 从可视化角度进一步说明了三类代表模型的差异。`MobileNetV3-Large` 对应“高性能前提下的大幅压缩”，`ResNet101` 对应“压缩与性能的均衡”，而 `DenseNet169` 对应“对 fixed-k 失配的纠偏”。
...
第二，不同 backbone 对 fixed-k 的适配程度可能不同。从当前结果看，`DenseNet169` 上的 fixed-k 更可能存在明显失配，因此 ACT 的纠偏作用更突出；而在 `ResNet101`、`MobileNetV3-Large` 等模型上，ACT 的收益则更多表现为在压缩子空间的同时尽量维持原有性能。这一判断仍需结合额外的谱分布或特征值统计进一步验证。

第三，平均值会受到个别模型的显著影响。本文中 `DenseNet169` 的大幅纠偏明显拉高了 ACT 的平均 AUROC；若剔除这一模型，ACT 相对 FedViM 的平均变化约为 Near `-0.40` 个百分点、Far `-0.79` 个百分点。此外，正文结果目前基于单次种子实验，小幅涨跌仍可能受到训练噪声、联邦划分和经验 `alpha` 校准的影响。

因此，从本章结果更适合得到的结论是：`ACT-FedViM` 在部分 backbone 上能够纠正 fixed-k 失配，在多数 backbone 上则主要体现为更自动、更紧凑的后处理选维。

### 5.6 本章小结

本章结果表明，`FedViM` 与 `ACT-FedViM` 在五个 CNN backbone 上都明显优于 `MSP` 与 `Energy`，说明联邦化 ViM 的主线是成立的。进一步地，ACT-FedViM 在所有 backbone 上都显著压缩了主子空间规模，并在五模型结果中保持了竞争性的 OOD 检测性能。就本章而言，更应强调的是：ACT 为联邦 ViM 提供了更自动、更轻量的后处理选维方式，并在部分模型上体现出对 fixed-k 失配的纠偏作用。
...
13. Han Y, He J, Xie C, et al. Benchmarking out-of-distribution detection for plankton recognition: A systematic evaluation of advanced methods in marine ecological monitoring[EB/OL]. arXiv preprint arXiv:submit/6905509, 2025.
