...
本文在 DYB-PlanktonNet OOD 数据集上对 `ResNet101`、`EfficientNetV2-S`、`MobileNetV3-Large`、`DenseNet169` 和 `ResNet50` 五个 CNN backbone 进行了评估。实验包含 `54` 个 ID 类别、`26` 个 Near-OOD 类别和 `12` 个 Far-OOD 类别，并采用 `5` 客户端、Dirichlet `alpha=0.1` 的联邦划分。五模型结果表明，`ACT-FedViM` 的平均 ID 准确率为 `96.48%`。相较于 fixed-k `FedViM`，其主子空间维度由平均 `804.8` 压缩到 `108.2`，平均压缩率达到 `86.2%`；同时，平均 Near-OOD / Far-OOD AUROC 分别达到 `95.64%` / `96.34%`，整体表现显著优于 `MSP` 与 `Energy` 基线。进一步分析显示，ACT 对平均 AUROC 的提升主要来自 `DenseNet169` 上对 fixed-k 失配的纠偏，而在其余 backbone 上，其更稳定的收益体现在主子空间压缩、跨 backbone 适配性和后处理轻量化方面。
...
Experiments are conducted on the DYB-PlanktonNet OOD dataset with five CNN backbones: `ResNet101`, `EfficientNetV2-S`, `MobileNetV3-Large`, `DenseNet169`, and `ResNet50`. The evaluation covers `54` in-distribution classes, `26` Near-OOD classes, and `12` Far-OOD classes under a `5`-client federated split with Dirichlet `alpha=0.1`. The five-model results show that `ACT-FedViM` achieves an average ID accuracy of `96.48%`. Compared with fixed-k `FedViM`, it reduces the average subspace dimension from `804.8` to `108.2`, corresponding to an average compression rate of `86.2%`, while reaching `95.64%` and `96.34%` average AUROC on Near-OOD and Far-OOD tasks, respectively. Both `FedViM` and `ACT-FedViM` outperform the `MSP` and `Energy` baselines by clear margins. Further analysis shows that the average AUROC gain is mainly driven by correcting fixed-k mismatch on `DenseNet169`, whereas the more consistent contribution of ACT lies in subspace compression, better cross-backbone adaptability, and deployment-friendly post-hoc inference.
...
相比之下，ViM[2] 同时利用特征空间结构和分类 logits，通过主子空间与残差空间分解刻画样本偏离 ID 分布的程度，更适合处理具有细粒度特征差异的视觉识别任务。本文选择 ViM 的直接依据，来自 Han 等[13] 针对 DYB-PlanktonNet 所构建的系统 benchmark：该研究统一评测了 `22` 种 OOD 检测方法，结果显示 ViM 在所构建基准上整体表现突出，并在 Far-OOD 场景中优势尤为明显。因此，本文将 ViM 作为联邦化实现的主方法，而将 `MSP` 与 `Energy` 保留为输出空间对照基线。对本文而言，ViM 还有一个直接优势：它所依赖的关键统计量是全局特征均值、协方差和分类 logits，而全局均值与协方差可以由客户端上传的一阶与二阶特征充分统计量在服务器端重构。这使得 ViM 不仅在任务属性上具有吸引力，也在联邦实现上具备天然适配性。
...
需要强调的是，这里的期望不是集中式服务器直接访问全部 ID 训练样本后计算得到，而是通过**联邦式后处理校准**获得。具体地，在主子空间 `P` 与全局均值 `\mu_{\text{global}}` 已经固定后，第 `i` 个客户端仅在其本地 ID 训练分片 `\mathcal{D}_i^{ID}` 上计算三项标量统计量：

$$
S_i^{E} = \sum_{x \in \mathcal{D}_i^{ID}} \text{Energy}(x), \qquad
S_i^{R} = \sum_{x \in \mathcal{D}_i^{ID}} \text{Residual}(x), \qquad
n_i = |\mathcal{D}_i^{ID}|.
$$
...
因此，`alpha` 的估计与前述 `\mu_{\text{global}}`、`\Sigma_{\text{global}}` 的重构在逻辑上属于同一条联邦后处理链：前者聚合的是一阶与二阶特征充分统计量，后者聚合的是在固定 `(f_\theta, P, \mu_{\text{global}})` 条件下得到的能量与残差标量统计量。两者都不需要上传原始图像或样本级特征，区别仅在于所服务的后处理环节不同。
...
因此，`k` 的显著降低能够直接减少投影矩阵存储开销和 OOD 打分时的矩阵乘法开销。本文五模型正式结果中，fixed-k FedViM 的平均 `k` 为 `804.8`，而 ACT-FedViM 的平均 `k` 为 `108.2`，后处理阶段的主子空间规模被压缩了约 `86.2%`。
...
第二，ACT-FedViM 的平均 AUROC 高于 fixed-k FedViM，但这种提升并不均匀，不应被直接表述为“ACT 稳定提升了所有模型的检测性能”。从逐模型结果看，ACT 在 Near-OOD 与 Far-OOD 上都只在 `2/5` 个模型上优于 fixed-k FedViM，在其余 `3/5` 个模型上出现不同程度的下降。

第三，平均提升主要由 `DenseNet169` 拉动。若将这一明显的 fixed-k 失配案例单独剔除，ACT 相对 FedViM 的平均变化约为 Near `-0.40` 个百分点、Far `-0.79` 个百分点。这说明 ACT 的价值不宜简单概括为“平均涨点”，而应理解为“在少数失配模型上纠偏，在多数模型上以压缩为主”。

第四，ACT-FedViM 显著改善了 fixed-k 的最坏情况表现。`FedViM` 在五模型中的最差 Near/Far-OOD AUROC 分别为 `80.04% / 84.78%`，而 `ACT-FedViM` 的最差 Near/Far-OOD AUROC 也分别达到 `95.26% / 95.89%`。这说明 ACT 虽不保证每个模型都涨点，但能够明显降低 fixed-k 失配带来的性能下界风险。
...
从表 5-2 和图 5-1 可以看出，`MSP` 与 `Energy` 作为纯输出空间基线，在当前细粒度浮游生物任务上明显弱于 ViM 系方法。以平均结果计，`FedViM` 相比 `MSP` 的 Near/Far-OOD AUROC 分别提升 `2.44` 和 `7.43` 个百分点，相比 `Energy` 分别提升 `9.51` 和 `13.82` 个百分点；`ACT-FedViM` 相比 `MSP` 的 Near/Far-OOD AUROC 分别提升 `5.22` 和 `9.08` 个百分点，相比 `Energy` 分别提升 `12.30` 和 `15.46` 个百分点。这说明本文工作的主要价值首先来自“将 ViM 成功联邦化”，其次才是“在联邦 ViM 上进一步引入 ACT 选维”。
...
这一案例最适合支撑本文“部署友好”的核心论点。对于需要长期部署在多中心监测流程中的 OOD 后处理模块而言，`k` 从 `512` 压缩到 `89` 的意义并不只是数字变化，更意味着投影矩阵存储量和投影计算开销的明显下降。同时，`MobileNetV3-Large` 也说明了一个关键事实：在某些已经表现很强的 backbone 上，ACT 的主要价值不是继续抬高 AUROC，而是在尽量保留性能的前提下把后处理模块做得更紧凑。

#### 5.3.2 ResNet101：压缩与性能兼顾的平衡案例

`ResNet101` 的 fixed-k 为 `1000`，ACT 选择 `143` 维，压缩率为 `85.7%`。在这一更小主子空间下，Near-OOD AUROC 从 `95.86%` 提升到 `96.28%`，Far-OOD AUROC 从 `96.68%` 变化到 `96.73%`。这一结果表明，在高维 CNN backbone 上，ACT-FedViM 可以在大幅缩小子空间规模的同时保持与 fixed-k ViM 基本一致的检测能力，并在正式结果中取得轻微增益。
...
`DenseNet169` 的结果与其他模型明显不同。其 fixed-k 为 `1000`，而 ACT 仅保留 `99` 维主子空间，压缩率达到 `90.1%`。在这一设定下，Near-OOD AUROC 从 `80.04%` 提升到 `95.57%`，Far-OOD AUROC 从 `84.78%` 提升到 `96.17%`。这表明在正式结果中，fixed-k 在该模型上很可能存在明显失配，而 ACT 通过统计驱动选维起到了较强纠偏作用。

`DenseNet169` 更适合被理解为 fixed-k 失配下的纠偏案例，而不是 ACT 在所有模型上的普遍收益。当固定 `k` 与具体 backbone 的谱结构严重不匹配时，ACT 可能带来非常明显的改善；而在其余模型上，ACT 的收益更多体现为压缩和稳健性。因此，这一结果的意义主要在于揭示 fixed-k 方案的潜在风险，并说明自适应选维在特定 backbone 上能够显著修复性能。

![图 5-3 正文代表模型对比](../../paper_tools/figures/figure_3_selected_models.png)

*图 5-3 `MobileNetV3-Large`、`ResNet101` 与 `DenseNet169` 三个代表模型上的 Near/Far-OOD AUROC 对比。*

图 5-3 从可视化角度进一步说明了三类代表模型的差异。`MobileNetV3-Large` 强调“高性能前提下的大幅压缩”，`ResNet101` 强调“压缩与性能的均衡”，而 `DenseNet169` 则强调“对 fixed-k 失配的纠偏”。三者共同构成了本文对 ACT-FedViM 价值边界的完整说明。
...
第三，ACT 的一致性压缩还带来了更稳的性能下界。尽管 `FedViM` 在部分模型上的绝对性能已经很高，但 fixed-k 的设定一旦与具体 backbone 不匹配，就可能出现明显退化；而 ACT 通过统计驱动方式选维，使得五个模型的最终 Near/Far-OOD AUROC 都维持在较高水平。这一性质对多 backbone 混合部署场景尤其重要。

### 5.5 为什么 ACT 没有统一提升所有模型

从表 5-1 可以看出，ACT-FedViM 并没有在所有 backbone 上都表现出一致的 AUROC 提升。结合方法结构，至少可以从四个角度理解这一现象。
...
第三，平均值本身会受到极端个例影响。本文中 `DenseNet169` 的大幅纠偏显著拉高了 ACT 的平均 AUROC，因此若只看平均数，很容易误以为 ACT 具有普遍涨点效应。实际上，它更多反映的是 ACT 在 fixed-k 失配条件下的风险修复能力。

第四，本文采用单次种子结果作为主报告。对于近似持平的模型，小幅涨跌仍可能受到训练噪声、联邦划分和经验 `alpha` 校准的影响，因此更适合被理解为个别模型表现，而不是稳定统计结论。

综合来看，`ACT-FedViM` 的主要作用在于为联邦 ViM 提供统计驱动的自适应选维，并以更小的主子空间保持竞争性的 OOD 检测性能。

### 5.6 本章小结

本章结果表明，`FedViM` 与 `ACT-FedViM` 在五个 CNN backbone 上都明显优于 `MSP` 与 `Energy`，说明联邦化 ViM 的主线是成立的。进一步地，ACT-FedViM 在所有 backbone 上都显著压缩了主子空间规模，并在五模型正式结果中保持了竞争性的 OOD 检测性能。从整体结果看，最应强调的不是“ACT 涨了多少点”，而是“ACT 让联邦 ViM 的后处理选维更自动、更轻量，并且降低了 fixed-k 失配导致的最坏情况风险”。
...
ACT-FedViM 在本文中的定位，不是替代 ViM 的全新 OOD 检测框架，而是在联邦 ViM 基础上引入 ACT 作为后处理自适应选维模块的轻量化增强方案。
...
第三，降低后处理部署成本并改善最差模型风险。ACT 并不改变联邦训练上传的统计量规模，但它显著缩小了部署阶段要保存的主子空间矩阵以及 OOD 打分时的投影复杂度。同时，从五模型结果看，ACT 明显抬高了方法在不同 backbone 上的性能下界，使系统更不容易因 fixed-k 失配而出现个别模型显著掉点。
...
2. 正文结果目前基于单次种子实验，尚未完成多种子统计，因此对小幅涨跌的解释仍需谨慎。
...
在 `54` 个 ID 类别、`26` 个 Near-OOD 类别和 `12` 个 Far-OOD 类别构成的 DYB-PlanktonNet OOD 数据集上，本文对 `5` 个 CNN backbone 进行了实验评估。五模型正式结果表明，ACT-FedViM 的平均 ID 准确率达到 `96.48%`；相较于 fixed-k FedViM，ACT 将主子空间维度平均从 `804.8` 压缩到 `108.2`，压缩率达到 `86.2%`；同时，平均 Near/Far-OOD AUROC 达到 `95.64% / 96.34%`，并整体显著优于 `MSP` 与 `Energy` 基线。

因此，本文的结论不是“ACT 一定优于 ViM”，而是：

1. ViM 可以通过充分统计量聚合被有效联邦化；
2. ACT 可以作为联邦 ViM 的自适应选维模块稳定工作；
3. ACT-FedViM 在显著压缩后处理主子空间规模的同时，保持了有竞争力的 OOD 检测能力，并降低了 fixed-k 失配导致的跨 backbone 最坏情况风险。
...
13. Han Y, He J, Xie C, et al. Benchmarking out-of-distribution detection for plankton recognition: A systematic evaluation of advanced methods in marine ecological monitoring[C]//*Proceedings of the IEEE/CVF International Conference on Computer Vision Workshops (ICCVW)*. 2025: 2142-2152.
