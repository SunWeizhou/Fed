#!/usr/bin/env python3
"""Evaluate the thesis-oriented independent FedLN baseline."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate the independent FedLN baseline.")
...
def main() -> None:
    ...
    print("\nCollecting logits for FedLN evaluation...")
    ...
    scorer = compute_msp_ood_scores if args.evaluation_score == "msp" else compute_energy_ood_scores
    ...
    payload = {
        ...
        "result_scope": config.get("result_scope", "supplemental_fedln_baseline"),
        "notes": config.get(
            "notes",
            "Thesis-oriented federated LogitNorm baseline evaluated with an output-space score.",
        ),
    }

    filename = "fedln_results.json" if args.evaluation_score == "msp" else f"fedln_results_{args.evaluation_score}.json"
