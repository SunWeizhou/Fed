#!/usr/bin/env python3
"""Collect independent FOSTER baseline results into standalone artifacts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect FOSTER baseline results.")
    parser.add_argument("--experiments-root", type=str, default="experiments/foster_v1")
    parser.add_argument("--output-prefix", type=str, default="paper_tools/foster")
...
def collect_records(root: Path) -> list[dict]:
    records = []
    for path in sorted(root.glob("*/experiment_*/foster_results*.json")):
...
def build_summary(records: list[dict]) -> dict:
    ...
    for model_name, model_records in by_model.items():
        preferred = sorted(
            model_records,
            key=lambda item: (item.get("evaluation_score") != "msp", item.get("timestamp", "")),
        )[0]
...
def main() -> None:
    ...
    print(f"Collected {len(records)} FOSTER result files from {root}")
