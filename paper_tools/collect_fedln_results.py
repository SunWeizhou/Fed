#!/usr/bin/env python3
"""Collect independent FedLN baseline results into standalone artifacts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect FedLN baseline results.")
    parser.add_argument("--experiments-root", type=str, default="experiments/fedln_v1")
    parser.add_argument("--output-prefix", type=str, default="paper_tools/fedln")
...
def collect_records(root: Path) -> list[dict]:
    records = []
    for path in sorted(root.glob("*/experiment_*/fedln_results*.json")):
...
def main() -> None:
    ...
    print(f"Collected {len(records)} FedLN result files from {root}")
